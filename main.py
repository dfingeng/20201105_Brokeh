...
if __name__ == '__main__':
    ...
    ib = IB()
    contract_USD = Forex('EURUSD')
    contract_AUDUSD = Commodity(conId=14433401, exchange='IDEALPRO', currency='USD')  # 澳元兑美元
    contract_Ag = Commodity(conId=77124483, exchange='SMART', currency='USD')
    contract_Au = Commodity(conId=69067924, exchange='SMART', currency='USD')
    Ibconnect()
    df = Get_Data(contract_Ag, dur, barsize1, endtime)
    ...
    p.vbar(df.date[dec], w, df.open[dec], df.close[dec], fill_color='#32CD32', line_color='black')
